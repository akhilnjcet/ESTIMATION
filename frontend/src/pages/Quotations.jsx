...
const Quotations = () => {
  ...
  const [previewData, setPreviewData] = useState(null); // Used for viewing existing quotation full-screen
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .doc-box { max-width: 800px; margin: auto; padding: 30px; border: 1px solid #eee; box-shadow: 0 0 10px rgba(0, 0, 0, 0.05); font-size: 16px; line-height: 24px; color: #555; }
          ...
          <div class="doc-box">
            ...
            <div class="details">
              ...
              <div style="text-align: right">
                ...
                <p style="font-weight: normal">Valid Until: <strong>${new Date(new Date(docData.createdAt).getTime() + 15*24*60*60*1000).toLocaleDateString()}</strong></p>
              </div>
            </div>
      ...
      <div className="card" style={{ background: '#fff', padding: isLive ? '1.5rem' : '3rem', color: '#000', margin: '0 auto', maxWidth: '800px', transform: isLive ? 'scale(0.95)' : 'none', transformOrigin: 'top center' }}>
        <div className="flex justify-between items-start mb-6 border-b pb-4" style={{ borderBottom: '2px solid #e2e8f0' }}>
          <div>
            ...
            <p style={{ color: '#64748b', margin: 0 }}>#{docData.quotationNumber}</p>
          </div>
          <div style={{ textAlign: 'right' }}>
            <h2 style={{ margin: 0, fontSize: isLive ? '1.2rem' : '1.5rem', color: selectedProgram?.themeColor || 'var(--primary)' }}>{selectedProgram?.name || 'Program Name'}</h2>
        ...
        <div className="flex justify-between mb-6">
          <div>
            <h3 style={{ color: '#64748b', textTransform: 'uppercase', fontSize: '0.75rem' }}>Quotation To:</h3>
          ...
          <div style={{ textAlign: 'right', fontSize: isLive ? '0.85rem' : '1rem' }}>
            ...
            <p style={{ margin: 0 }}><strong>Valid Until:</strong> {new Date(new Date(docData.createdAt).getTime() + 15*24*60*60*1000).toLocaleDateString()}</p>
          </div>
        </div>
        ...
        <div style={{ width: isLive ? '250px' : '300px', marginLeft: 'auto', fontSize: isLive ? '0.85rem' : '1rem' }}>
          ...
          <div className="flex justify-between mt-3 pt-3" style={{ borderTop: '2px solid #e2e8f0', fontSize: isLive ? '1.1rem' : '1.25rem', fontWeight: 'bold' }}>
            <span>Total:</span>
            <span style={{ color: 'var(--primary)' }}>₹{(docData.totalAmount || 0).toLocaleString()}</span>
          </div>
        </div>
    ...
    return (
      <div className="preview-overlay" style={{ background: '#f1f5f9', minHeight: '100vh', padding: '2rem' }}>
        <div className="flex justify-between items-center mb-4 print-hide">
          ...
          <button className="btn btn-primary" onClick={() => handlePrint(previewData)}>Print PDF</button>
        </div>
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="btn btn-primary" onClick={() => setShowForm(!showForm)}>{showForm ? 'Cancel' : 'Create Quotation'}</button>
      </div>
      ...
      {showForm && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginBottom: '2rem', alignItems: 'start' }}>
          ...
          <div className="card" style={{ borderTop: '4px solid var(--primary)', position: 'sticky', top: '1rem' }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="dashboard-grid">
                  ...
                  <div className="form-group">
                    ...
                    <input type="text" className="form-control" value={formData.terms} onChange={e => setFormData({...formData, terms: e.target.value})} placeholder="Payment within 15 days..." />
                  </div>
                </div>

                <button type="submit" className="btn btn-primary" style={{width:'100%', padding:'1rem', fontSize:'1rem'}}>Save & Generate Quotation</button>
              </form>
            )}
          </div>
            ...
            <tbody>
              {quotations.map(q => (
                <tr key={q._id}>
                  ...
                  <td><span style={{ color: 'var(--warning)', padding: '0.2rem 0.5rem', background: '#fffbeb', borderRadius: '4px' }}>{q.status}</span></td>
                  <td>
                    <button className="btn btn-secondary" onClick={() => setPreviewData(q)} style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem', marginRight: '0.5rem' }}>View / Print</button>
                  </td>
                </tr>
              ))}
              {quotations.length === 0 && <tr><td colSpan="6" style={{textAlign:'center'}}>No quotations found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
